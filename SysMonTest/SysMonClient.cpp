	...
	while (count > 0)
	{
		...
		switch (header->Type)
		{
			...
			case ItemType::ProcessCreate:
			{
				...
				std::wstring imageFileName((WCHAR*)(buffer + info->ImageNameOffset), info->ImageNameLength);
				printf("Process %d Created.\t Process Name:%ws\t Command line: %ws\n\n\n", info->ProcessId, imageFileName.c_str(), commandline.c_str());
				break;
			}
